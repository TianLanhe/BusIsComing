...
- Google 地圖移除端點直線步行示意；成功分段只沿 CSDI 各有序子路徑及局部切線繪製較粗灰色開放折角，不顯示灰色實線、點線或虛線底圖；局部失敗不補畫假路徑。實際展示地政總署資料時，左下署名壓縮至約 116×29dp、官方標誌約 15dp，保留既有雙行內容與完整可開啟說明，並以獨立 48dp 有效觸控區兼顧精巧外觀和可操作性。
...
- 需覆蓋三語、TalkBack、窄屏／大字體、亂序 callback、配置重建、部分失敗、排序移動、地圖子路徑，以及署名約 116×29dp 可見尺寸、15dp 標誌、字體放大上限、48dp 有效觸控區與 Google attribution 安全區；真實 Citybus／CSDI 只讀抽查受外部服務可用性限制，不能成為一般自動測試依賴。
