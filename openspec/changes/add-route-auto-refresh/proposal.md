...
- **前置依賴**：詳情自動刷新以 `fix-route-detail-progressive-loading` 的 reducer、domain generation、可靠結構快取及互動狀態保持為基線；本 change 可先完成提案，但實作與歸檔須在該 change 同步及歸檔後進行。由於該新能力目前尚未進入主規格，詳情定時刷新契約由本 change 的 `route-auto-refresh` 新能力承載，不建立虛假的 MODIFIED delta。
