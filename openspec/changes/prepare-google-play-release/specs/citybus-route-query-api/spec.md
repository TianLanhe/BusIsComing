...
### Requirement: 保留请求头和 Cookie
**Reason**: 已用每接口 10 個有效 live 請求確認 Citybus mobile 路線查詢不需要靜態瀏覽器 header 或 cookie；保留 copied browser headers/cookies 會增加隱私、追蹤與維護風險。

**Migration**: `ppsearch_p3.php` 改為無顯式 header 的 GET 請求；若未來上游新增限制，只針對失效接口加入最小必要 header，不恢復靜態 cookies。
