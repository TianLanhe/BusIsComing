...
### Requirement: 保留请求头
**Reason**: 已用 10 個有效 live 請求確認 Citybus 地點搜索不需要靜態瀏覽器 header 或 cookie；保留 `Cookie`、`Referer`、`User-Agent` 和 `X-Requested-With` 沒有功能必要性。

**Migration**: `bsearch_p3.php` 改為無顯式 header 的 GET 請求；解析和 UI 狀態處理保持不變。
