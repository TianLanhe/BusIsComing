...
- **AND** 系統 SHALL 在 `全部` 入口旁展示獨立的 `管理路線` 圖示入口
...
- **AND** `管理路線` 入口 SHALL 使用編輯／管理列表語義的圖示按鈕
- **AND** `管理路線` 圖示 SHALL 提供無障礙描述 `管理路線`
...
- **AND** `常用路線` 標題、快捷卡、`全部` 入口、管理圖示、首次引導主文案、示例預覽、主要操作入口和臨時查詢結果區 SHALL NOT 與頂部按鈕重疊
