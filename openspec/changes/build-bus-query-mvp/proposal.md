...
BusIsComming 需要先完成一个可用的 Android MVP，让用户能够保存常用的起点/终点地址，并快速查看这些路线的可选巴士方案。当前项目仍是空的 Android Studio 工程，因此这个变更用于建立 App 的第一版产品界面、本地数据模型和查询流程。
...
- 会在 `com.example.busiscomming` 下新增数据模型、本地存储、仓库和 UI 页面相关 Kotlin 代码。
