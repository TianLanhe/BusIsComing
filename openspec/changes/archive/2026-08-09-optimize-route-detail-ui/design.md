...
路線詳情目前由 `RouteDetailActivity` 協調 `MapView`、三檔 `BottomSheetBehavior`、並發資料域及列表互動；`RouteMapPresentationBuilder` 生成純 Kotlin marker／line model，`GoogleRouteMapRenderer` 以 marker、帶描邊巴士 polyline 及方向折角差量繪製；`RouteDetailUiFormatter` 和 `RouteDetailAdapter` 產生摘要與時間線。現有 `fix-route-detail-progressive-loading` change 已把詳情載入改為 generation-aware 的資料域 reducer、可靠結構門禁、single-flight 及可分域快取，本 change 必須建立在該行為上，不得重做或倒退。

原始 UI 的主要限制是：marker 仍由 Canvas 基本幾何形狀產生；普通站名依賴 Google marker info window 而沒有碰撞治理；巴士線沒有前進方向，步行仍是灰色虛線；摘要沒有完整可點擊行動鏈；乘車段仍顯示邊框、首程 ETA 及單段站數；全屏仍顯示 Toolbar。首次實作後又暴露兩個驗收問題：`onSlide()` 逐幀連鎖觸發 CSDI `layoutParams`、`GoogleMap.setPadding()`、全量方向 marker remove／add 與站名 bitmap 重建；方向 placement 以整條投影線總長度套用每線 80 個上限，使高 zoom 時屏外長度稀釋可見密度，且過大的折角在彎道越出 7dp 色線。
...
- 讓方向密度只由可見視口與固定屏幕間距決定，並在拐角安全窗口不足時省略而非越線。
- 讓 bottom sheet 連續拖動不逐幀重建地圖 marker、文字 bitmap 或 CSDI layout。
...
巴士段仍由 9dp 白色 outline polyline 加 7dp 分段色 core polyline 組成。方向層由 renderer 在 camera idle 或穩定檔位 padding 更新後取得 Google Maps `Projection`，只把目前安全視口及少量 overscan 相交的同一有序 geometry 投影成屏幕折線。巴士白色淺折角的完整視覺包絡約 5.5dp、stroke 約 1.2dp、固定間距約 36dp，所有非透明像素必須收在 7dp 色線內；成功 CSDI 步行不繪製任何灰色承載線，只以約 9dp、stroke 約 2.4dp、固定間距約 14dp 的灰色開放折角展示 path。

固定直線、急彎、S 彎及反向點序的最小裝置實驗先驗證 SDK stamp/style；該路徑因重採樣形成密集鋸齒而失敗。修訂後的屏幕投影方案須繼續通過：

1. 步行只留下粗灰折角而無底線；
2. 折角在直線及拐角保持固定視覺尺寸與已確認間距，屏外幾何不稀釋目前視口密度；
3. 每個折角嚴格貼合所在屏幕線段的局部切線；
4. 反向 geometry 使全部折角反轉；
5. 縮放及 padding 改變後在 camera idle 重排，不沿用舊角度或舊位置。

每個候選位置前後必須具有足以容納完整 glyph 的同一局部切線窗口；窗口跨越角度變化過大的 vertex 時移至鄰近安全直線區間，沒有安全區間時省略而不使用任一單邊 segment 冒充。安全上限只在 viewport clipping 後限制異常資料，不能反向放大正常 spacing。每條 rendered line 維護方向 marker pool，重排時按序復用 marker 並只更新 position／rotation／icon，只新增不足數量及移除超額數量；方向 bitmap 依 density、kind 與 palette cache。

不得使用整段起終點 bearing、固定角度 bitmap、逐幀 camera move／bottom sheet slide 重排或脫離 geometry 的手工位置。若 projection 暫不可用或某段不能產生可靠局部切線，巴士保留可靠實線、步行省略該段折角。
...
可見 segment 使用 content-wrap、約 2dp gap、30dp 底色高度及共同底部 baseline；現有 `ic_walking_person` 由 18dp 等比放大至 24dp，路線號約 17sp，小號耗時約 12sp。水平容器單行捲動。每個可見塊保留獨立語義節點，TouchDelegate 只在不增加可見寬度的前提下向上下擴張至至少 48dp 有效操作高度；水平方向以可見塊邊界或相鄰中點裁切，不能互相覆蓋或增加大段空白。第一行總耗時／預計到達保持目前 21sp、字重與排版，第三行統計保持目前 13sp 與間距。
...
- instrumentation／view：segment 30dp 可見高度、24dp 真實步行圖標、第一／三行字級不變、28dp 可見把手、至少 48dp 有效操作高度且水平方向不重疊、水平捲動、baseline、adapter 無邊框、fare／ETA 去重、pending target、focus／announcement 及 full-screen chrome；
- 地圖裝置：固定直線、急彎、S 彎及反向 geometry 的 viewport-local 折角、無底線步行、不同 zoom／stable padding／漸進更新、marker pool、色線內包絡與站名避讓；
...
### 11. bottom sheet 拖動使用兩階段地圖更新

性能採樣確認 `RouteDetailActivity.onSlide()` 目前逐幀呼叫 `updateMapPadding()`，再修改 CSDI margin、呼叫 `GoogleMap.setPadding()` 並全量重建方向與站名；卡頓是 App 主動製造的 renderer churn，而不是 MapView 單純存在。sheet 進入拖動時只做一次狀態切換：把既有 label marker 淡出但不移除，記錄已提交 detent／padding，保留路線、方向、站點、相機及選中狀態。

拖動 frame 只以 `translationY` 讓 CSDI 署名跟隨 sheet，不修改 `layoutParams`。向上時最多預留一次下一 detent 的安全 padding；向下時保留起始較大 padding，同一候選值不重複呼叫 `GoogleMap.setPadding()`。到達 SUMMARY／HALF／FULL 後才清除 translation、提交一次正式 margin 與精確 padding、重排一次可見方向及站名並淡入。取消、直接跳檔、Map 不可用或 Activity 銷毀均只接受最後 stable state，不執行過期 callback。

否決每個 frame 只做 coalescing 的方案：即使降至每個 vsync 一次，marker remove／add、bitmap 與 layout 仍在手指移動期間競爭主線程。也否決 Canvas overlay 重寫，因本輪可用兩階段更新和 marker pool 解決，無須引入第二套 hit testing／projection 生命週期。

### 12. 把手縮小而保留有效操作範圍

`routeDetailSheetHandle` 的可見版面高度由 48dp 改為 28dp，36×4dp 橫線居中，上下各約 12dp；sheet content 以透明 TouchDelegate 向下擴展至至少 48dp，擴展區只覆蓋不可點擊的標題空間，不遮擋摘要階段。三 detent 點擊切換、BottomSheetBehavior 拖動及單一 TalkBack 節點保持不變。把手減少 20dp、摘要階段增加 8dp 後，普通摘要態總高度仍比目前少約 12dp 非必要空白。
...
- [30dp 可見塊與 48dp 觸控高度仍可能產生重疊] → 視覺層與語義觸控層分離，只向上下擴張 TouchDelegate，水平方向按可見塊邊界或相鄰中點裁切，並加入坐標級 instrumentation 測試。
- [拖動時預留 padding 令地圖可用區短暫偏小] → 只在過渡期使用較保守安全矩形，stable detent 後立即精確恢復，換取零 marker churn 與不遮擋。
- [較密步行 marker 增加 GPU 負載] → 只在 viewport＋overscan 生成，camera move／sheet slide 不重排，異常 cap 在 clipping 後才生效。
...
6. 先以測試鎖定 viewport placement、拐角窗口、marker pool、兩階段 sheet transition、28dp 把手及 30dp 摘要，再完成首次實作驗收修正。
7. 運行定向測試、`./gradlew build` 與任務自有 AVD 視覺／TalkBack／拖動性能矩陣；未通過方向貼合或仍逐幀重建 marker 即不視為完成。
...
無未確認產品決策。Maps SDK stamp 的裝置實驗已失敗；首次屏幕投影實作已完成，後續須按本次確認的 viewport-local 密度、線內包絡、拐角安全、marker pool 與兩階段拖動更新重新驗收。
