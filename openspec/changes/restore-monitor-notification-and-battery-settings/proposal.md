...
- `notification-bar-monitoring`: 在啟動流程展示可操作的通知／鎖屏準備狀態，並在設定返回後重新檢查再決定是否啟動。
- `monitor-high-priority-scheduling`: 恢復直接電池最佳化豁免檢查、manifest 權限與系統確認頁，同時保留拒絕時的非阻塞降級。

## Impact

- 受影響代碼：`service/BusMonitorNotificationContract.kt`、新增或抽取的 notification channel 狀態／導航 helper、`service/BusMonitorSchedulingCapability.kt`、`ui/main/MainActivity.kt`、`ui/main/MonitorSettingsBottomSheet.kt` 與 `AndroidManifest.xml`。
- 受影響資源：繁體、簡體與英文的監控準備狀態、設定操作、回退引導及電池豁免說明文案。
...
- 測試：以純狀態判斷與可注入 navigator 單測覆蓋 App 關閉、channel 缺失／停用／鎖屏隱藏、直接跳轉、逐級回退、返回重查、電池已豁免／未豁免及 Intent 失敗。
