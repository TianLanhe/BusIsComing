...
- [x] 1.1 在網站倉庫確認 `GET /api/downloads/android/latest/metadata` 已註冊並返回 `platform`、`status`、`applicationId`、`versionName`、`versionCode`、`fileName`、`sizeBytes`、`lastUpdated`、`downloadUrl`，以契約測試確認 `applicationId=com.golink.busiscoming`、HTTPS 下載 URL 及 `Cache-Control: no-store`；若服務尚未部署，記錄網站渠道真實驗收的阻塞證據，不得在 App 內硬編碼最新版本。
...
- [x] 2.2 新增可注入 clock 的 `UpdatePolicy`，集中定義 24 小時自動檢查、3 天首次提醒及 3 天稍後提醒；以 JVM 測試覆蓋未滿／剛好／超過門檻、香港完整日及系統時間回撥。
...
- [x] 3.3 新增 `WebsiteUpdateSource` 透過 HTTPS 讀取固定 metadata endpoint，驗證 host、platform、available 狀態、application ID、正整數 versionCode、versionName、日期及必要欄位；以 fixture 測試合法、無更新、缺欄位、錯誤 host、錯誤 App、錯誤日期與 HTTP 失敗。
...
- [ ] 8.3 核對本 change 的 requirement／scenario 均有實作或驗證證據，同步勾選完成任務；執行 `git status --short` 與 staged diff 檢查，避開既有無關改動後依專案規則建立單一清晰 conventional commit。
