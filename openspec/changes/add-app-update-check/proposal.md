...
- **外部系統**：依賴 Google Play Core、Play 商店詳情頁、網站 metadata endpoint 與三語首頁 `#download`。網站 endpoint 必須提供並驗證 `applicationId`、`versionCode`、`versionName`、`lastUpdated` 等白名單欄位；網站實作與部署屬跨倉庫前置條件。
...
- **驗證**：需要純邏輯單測、設定頁 instrumentation、三語×深淺色×360dp×大字體與 TalkBack 驗收、無 Play 模擬器網站流程，以及具有相同 application ID／簽名和更高 versionCode 的 Google Play internal test 真實 flexible update 驗證；Android 實作完成後運行 `./gradlew build`。
