...
- [ ] 1.3 以 `play-services-maps 20.0.0` 的 `StrokeStyle`／`StyleSpan`／`SpriteStyle` 建立最小 instrumentation spike，固定渲染直線、急彎、S 彎及反向有序 geometry，並覆蓋彩色巴士 stroke 與透明步行承載 stroke。
- [ ] 1.4 在裝置上核對折角只沿同一 polyline 局部切線定向、反向 geometry 全部反轉、步行只留下粗灰折角、縮放／padding 後間距穩定且拐角不漂移；保存不含敏感資料的可復核畫面及結果。
- [ ] 1.5 若 1.4 任一硬條件失敗，停止後續正式 UI 實作、保持 tasks 未完成並回到 design/spec 討論；不得加入整段 bearing、手工角度、獨立 Marker 或固定方向 bitmap 回退。
...
- [ ] 4.1 擴展 `RouteMapMarkerIconFactory`／renderer bitmap cache，以角色、色 slot、前後段色、模式、選中狀態及 density bucket 產生固定比例 marker，並測試相同 key 復用與狀態改變失效。
- [x] 4.2 以通過 spike 的 polyline stamp/style 實作巴士白描邊＋分段色實線＋白色開放折角，確保同一有序 geometry、stable id 差量更新及反向定向。
- [x] 4.3 只對 CSDI 成功分段的每個有序 path 使用透明承載 stroke＋粗灰開放折角；移除端點直線與舊 `Dash／Dot／Gap` 地圖 pattern，Loading／失敗／Citybus fallback／SameStop 不得建立步行 line 或補線。
- [x] 4.4 為 stamp 建立失敗加入安全診斷與降級：巴士保留可靠實線、步行省略紋理；測試不得建立手工 Marker 或顯示方向錯誤的折角。
...
- [ ] 4.7 保留 marker／時間線雙向聯動、選中途經站展開、相機所有權、全覽、目前位置、MapView 生命週期及地圖／geometry 獨立降級，補充差量更新與過期 generation 回歸測試。
...
- [ ] 5.4 為每個可見分段建立至少 48dp 且互不重疊的透明操作／語義範圍，以相鄰中點裁切 TouchDelegate；測試觸控座標邊界及 TalkBack 行程順序。
...
- [ ] 5.7 在窄屏、大字體及長路線號下驗證行動鏈只水平捲動而不換行／壓縮圖標，第三層則可自然換行且不裁切、縮字或形成內部垂直捲動。
...
- [ ] 7.2 運行受影響 instrumentation 測試，覆蓋 22dp 可見／48dp 操作、觸控不重疊、水平捲動、分段跳轉／高亮／焦點／朗讀、時間線 rail、票價／ETA 去重與全屏 chrome。
...
- [ ] 7.5 在固定直線、急彎、S 彎、反向 geometry、不同 zoom 與 bottom sheet padding 下再次做最終方向驗收；任何折角未嚴格貼合局部軌跡或拐角漂移均視為未完成。
- [ ] 7.6 完成繁體／簡體／英文、淺／深色、約 360dp／窄屏、font scale 1.0／1.3／2.0 與 TalkBack 矩陣，確認第三方名稱原文、Google attribution、安全區、48dp 操作及列表終點可達。
