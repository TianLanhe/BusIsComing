...
系統 SHALL 讓主查詢頁、路線管理頁和路線編輯頁在淺色及深色模式下均採用 `docs/ui-style-guide.md` 中定義的「安靜實用的現代通勤工具」風格。

#### Scenario: 主查詢頁使用統一風格
- **WHEN** 用戶打開主查詢頁
- **THEN** 頁面 SHALL 使用清晰標題、路線選擇、主查詢按鈕、狀態區域和結果卡片組成第一屏
- **AND** 頁面 SHALL 使用模式對應的專案主色、輔助色、語意表面、克制圓角和清楚字號層級

#### Scenario: 路線管理頁使用統一風格
- **WHEN** 用戶打開路線管理頁
- **THEN** 頁面 SHALL 與主查詢頁保持一致的模式對應背景、標題層級、按鈕層級、卡片樣式和間距節奏
...
- **WHEN** 用戶在淺色模式打開主查詢頁、路線管理頁或路線編輯頁
- **THEN** 頁面 SHALL 保持現有淡綠到近白的克制漸變背景
- **AND** 卡片、狀態區、排序 chips、表單和候選列表 SHALL 保持現有白色或淺色實體表面層級
- **AND** 系統 SHALL NOT 因語意色整理而改變既有資訊架構、間距、字體、圓角或排序

#### Scenario: 深色模式使用深青綠根背景
- **WHEN** 用戶在深色模式打開主查詢頁、路線管理頁或路線編輯頁
- **THEN** 頁面 SHALL 使用「深青綠夜行」方向的深青綠到近黑根背景
- **AND** 卡片、狀態區、排序 chips、表單和候選列表 SHALL 使用比根背景清楚可辨識的深色實體表面
...
- **WHEN** 系統顯示臨時查詢、路線詳情、ETA、監控設定 Bottom Sheet，或保存、刪除及匯入確認 Dialog
...
系統 SHALL 讓所有 App 自有 Activity、RecyclerView item、Drawable、動態 View、Material 元件及系統欄使用目前外觀模式對應的語意色。
...
- **WHEN** 系統以 Kotlin 動態建立常用路線選擇列、臨時查詢、ETA、詳情或監控元件
