...
目前 `Theme.BusIsComing` 繼承 `Theme.MaterialComponents.DayNight.NoActionBar`，但 `app/src/main/res/values-night/themes.xml` 只覆蓋少量 Material 主色；App 自有的 `bus_*` 色彩全部位於 `values/colors.xml`。所有 layout、Drawable 和多個 Kotlin 動態元件因此在日夜模式下取得相同的頁面、卡片、文字與描邊色。

已知固定日間色包括 `activity_main.xml` 的白色按鈕表面、`table_row_background.xml` 的 `@android:color/white`，以及 `RouteDetailBottomSheet` 的 `Color.WHITE` 根背景。Material Dialog／Bottom Sheet 和 App 自有 View 由兩套色彩來源渲染，造成深色模式層級不一致。

設定頁由 `ui/settings/SettingsActivity.kt` 與 `activity_settings.xml` 承載，`偏好` 目前只有尚未支援的語言入口。專案已有 SharedPreferences store 的輕量模式，但沒有自訂 Application 在 Activity 建立前套用外觀偏好。
...
- 不新增跨 Activity 的查詢結果持久化或 ViewModel 架構重寫。
...
- 新增根 package 的 `BusIsComingApplication.kt`。Application `onCreate()` 讀取 store 並在任何 Activity inflate 前呼叫 `AppCompatDelegate.setDefaultNightMode()`；Manifest 透過 `android:name` 註冊。

原因：模式映射可純單測；持久化不依賴 Activity；程序冷啟動不會先以錯誤主題繪製再切換。SharedPreferences 足以保存單一小型 enum，不需要新增 DataStore 依賴或遷移。
...
### Decision 2: 設定頁使用含摘要的單列與 Material 單選對話框

做法：在 `偏好` 分組第一項新增 `外觀主題`，右側或次要文字顯示已保存值；現有 `語言` 位於其後。點擊後以 `MaterialAlertDialogBuilder` 或等效 Material 單選 API 顯示：

1. `跟隨系統`
2. `淺色模式`
3. `深色模式`

打開時選中 store 中的模式。選擇不同模式時先保存，再呼叫 `AppCompatDelegate.setDefaultNightMode()`；畫面重建和摘要更新就是成功反饋，不顯示 Toast。選擇目前模式只關閉對話框，不重建。
...
- 系統狀態列與導覽列使用模式對應背景及圖示明暗；API 26 才支援的 light navigation bar 屬性放入適當 `values-v26`／夜間限定資源，API 25 使用能配合系統能力的深色欄背景與淺色圖示。
...
- 主頁進行中的網路查詢、已顯示的即時 ETA／結果。
- 已打開的臨時查詢、ETA、詳情或監控 Bottom Sheet。

原因：使用框架標準路徑能避免同一 Activity 內混用舊新資源。即時 ETA 本身具有時效，重新查詢比新增跨 Activity 序列化更可靠。
...
- [Risk] API 25 與近期 edge-to-edge 系統欄行為不同。→ Mitigation：按 API 能力限定 theme 資源，至少在 Android 7.1 與近期版本驗證 status／navigation bar 圖示可見性。
...
2. 新增 `AppThemeMode`、store、Application 與 Manifest 註冊，確認無偏好時保持跟隨系統。
3. 擴充設定頁與字串，接入單選對話框和立即套用。
4. 建立日／夜色票與 Material 主題映射，更新風格指南。
5. 逐頁清理固定日間表面，先處理已知 XML／Drawable，再處理所有 Kotlin 動態 UI。
6. 執行相關單測、UI／contract 測試、`./gradlew build` 和模擬器／實機矩陣。
