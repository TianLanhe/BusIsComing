...
系統 SHALL 使用未加密 UTF-8 JSON 作為 `.bicroutes` 版本 1 內容，並 SHALL 嚴格區分可分享路線定義與本機狀態。
...
#### Scenario: 匯出單條路線欄位
- **WHEN** 系統將一條常用路線寫入版本 1 文件
- **THEN** 該路線 SHALL 只包含 `name`、`origin` 與 `destination`
...
- **AND** 文件 SHALL NOT 包含查詢結果、ETA、定位、排序快取或通知監控 session
