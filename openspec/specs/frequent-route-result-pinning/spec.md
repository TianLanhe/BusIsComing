# frequent-route-result-pinning Specification

## Purpose
定義常用行程查詢結果的本次與長期置頂能力，涵蓋嚴格路線身份、置頂順序、普通排序邊界、SQLite 持久化、生命週期、橫滑互動、錯誤恢復、三語文案與無障礙要求。
...
#### Scenario: 長期置頂再次向右
- **WHEN** 用戶把長期置頂路線卡向右滑動
- **THEN** 卡片 SHALL 在透明底層顯示已長期置頂文字後回彈
...
系統 SHALL 以有限 pull-to-action 讓路線卡水平跟手，依目前語言動作文字的實際寬度決定觸發門檻，且 SHALL 排除 ETA 與通知監控的既有觸控區。
...
#### Scenario: 文字完全露出即達門檻
- **WHEN** 系統為目前方向顯示置頂、升級、取消、已長期置頂或不可置頂文字
- **THEN** 觸發距離 SHALL 由卡片邊緣 16dp 留白加目前字型實際量測的本地化文字寬度組成
- **AND** 系統 SHALL NOT 使用卡片寬度百分比作為觸發門檻

#### Scenario: 卡片滑動具有有限盡頭
- **WHEN** 用戶繼續拖動並超過觸發距離
- **THEN** 卡片可見位移 SHALL 最多為觸發距離加 8dp
- **AND** 卡片 SHALL NOT 繼續移出畫面或被視為已 dismiss

#### Scenario: 未達門檻平滑回彈
- **WHEN** 用戶釋放卡片時動作文字尚未完全露出
- **THEN** 卡片 SHALL 在約 180–240ms 內平滑回彈
- **AND** 系統 SHALL NOT 改變置頂狀態

#### Scenario: 達門檻先回彈後執行
- **WHEN** 用戶在動作文字完全露出後釋放卡片
- **THEN** 卡片 SHALL 先回到零水平位移
- **AND** 系統 SHALL 只在回彈完成後執行對應置頂狀態動作或提示
- **AND** ItemTouchHelper SHALL NOT 將卡片移除或留下待清理滑動狀態
...
#### Scenario: 所有滑動方向使用透明底層
- **WHEN** 卡片因任何可用或不可用置頂手勢露出底層
- **THEN** 系統 SHALL 直接露出結果頁既有背景且不繪製彩色、灰色或警示色矩形
- **AND** 系統 SHALL 只以適配目前明暗主題的主要文字色繪製本地化動作文字

#### Scenario: 連續兩次右滑後卡片保持可見
- **WHEN** 同一路線先由未置頂右滑為本次置頂，再次右滑升級為長期置頂
- **THEN** 卡片 SHALL 保持在原置頂 token 所決定的位置並顯示長期書籤
- **AND** 卡片 SHALL 具有零水平位移、正常高度及可操作內容
- **AND** 清單 SHALL NOT 留下空白卡位或只剩滑動提示文字
...
### Requirement: 置頂移動採用符合動作語義的長列表視口
系統 SHALL 以穩定 item identity 及差異動畫移動置頂或取消置頂卡片，並 SHALL 讓首次置頂顯示新的置頂區頂部，而其他更新保持適當閱讀位置。

#### Scenario: 深處卡片置頂
- **WHEN** 用戶在長列表較深位置置頂一張卡片
- **THEN** 系統 SHALL 將卡片資料位置移入置頂區域
- **AND** 該卡片 SHALL 成為 LIFO 置頂區第一張卡片
- **AND** 清單 SHALL 在移動後以短距離平滑方式顯示結果頂部，使新置頂卡片可見並讓原有卡片向下推移

#### Scenario: 本次置頂升級不強制回頂
- **WHEN** 用戶把本次置頂升級為長期置頂
- **THEN** 系統 SHALL 保留相同穩定 identity、token、位置及目前閱讀視口
- **AND** 系統 SHALL NOT 因只改變置頂等級而再次捲到結果頂部
