...
- **路線匯入與匯出**：在設定頁以 `.bicroutes` 文件匯出全部常用路線，或預覽後合併／取代本機路線。
...
常用路線保存在 App 的本機 SQLite 資料庫。使用「設定 → 路線資料 → 匯入與匯出常用路線」可建立版本化的 `.bicroutes` 文件；文件是未加密的 UTF-8 JSON，只包含路線名稱、起點及終點的地點名稱與精確座標，不包含使用次數、最近使用時間、查詢結果、ETA、定位狀態或通知監控資料。請只把匯出文件交給信任的人。

匯入及匯出使用 Android 系統文件選擇器，不要求外部儲存權限。此功能只處理本機常用路線，不改變 Citybus mobile 或 DATA.GOV.HK ETA 的生產資料來源。
