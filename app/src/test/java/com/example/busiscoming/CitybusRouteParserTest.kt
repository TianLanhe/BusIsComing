...
import com.example.busiscoming.data.repository.CitybusRouteParseException
...
class CitybusRouteParserTest {
    ...
    fun parsesSingleLegFirstEtaQueryFromShowroutep2p() {
        ...
        val detailQuery = routes.first().routeDetailQuery
        assertEquals("1|*|CTB||8X-THR-1||6||20||O|*|", detailQuery?.rawInfo)
        assertEquals("0", detailQuery?.listId)
        assertEquals("12:00|*|30", detailQuery?.generalInfo)
        assertEquals("0", detailQuery?.lang)
        assertEquals("1|*|CTB||8X-THR-1||6||20||O|*|", detailQuery?.plan?.rawInfo)
        assertEquals("0", detailQuery?.plan?.lang)
        assertEquals(1, detailQuery?.plan?.legs?.size)
    }
    ...
    fun parsesOnlyFirstLegFromMultiLegShowroutep2p() {
        ...
        val detailLegs = routes.first().routeDetailQuery?.plan?.legs.orEmpty()
        assertEquals(2, detailLegs.size)
        assertEquals(listOf("8X", "1"), detailLegs.map { it.route })
        assertEquals(listOf(6, 5), detailLegs.map { it.boardingSeq })
        assertEquals(listOf(31, 15), detailLegs.map { it.alightingSeq })
    }
    ...
    fun parsesRealCandidateShowroutep2pAttribute() {
        ...
        assertEquals("0", routes.first().routeDetailQuery?.listId)
        assertEquals("21:54|*|93", routes.first().routeDetailQuery?.generalInfo)
        assertEquals("2|*|CTB||8X-THR-1||6||31||O|*|CTB||1-MAF-1||5||15||I|*|", routes.first().routeDetailQuery?.rawInfo)
    }

    @Test
    fun rejectsMalformedP2pPlan() {
        assertEquals(null, CitybusRouteParser.parseP2pRoutePlan("2|*|CTB||8X||6||9||O|*|"))
    }
