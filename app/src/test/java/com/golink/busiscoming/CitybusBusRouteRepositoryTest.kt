...
import com.golink.busiscoming.data.repository.RouteCardStopPreviewResolver
...
class CitybusBusRouteRepositoryTest {
    ...
    @Test
    fun logsRedactedSummaryForEachModeRequest() {
        ...
        assertEquals(setOf("T", "F", "W"), logs.map { it.summaryValue("searchMode") }.toSet())
        logs.forEach { summary ->
            assertTrue(summary.contains("endpoint=ppsearch_p3.php"))
            assertTrue(summary.contains("origin=present"))
            assertTrue(summary.contains("destination=present"))
            assertTrue(summary.contains("headers=0"))
            assertFalse(summary.contains("curl"))
            assertFalse(summary.contains("https://mobile.citybus.com.hk/nwp3/ppsearch_p3.php"))
            assertFalse(summary.contains("Cookie"))
            assertFalse(summary.contains("User-Agent"))
            assertFalse(summary.contains("slat="))
            assertFalse(summary.contains("slon="))
            assertFalse(summary.contains("elat="))
            assertFalse(summary.contains("elon="))
            assertFalse(summary.contains("2026-06-03"))
            assertFalse(summary.contains("22.267079693838"))
            assertFalse(summary.contains("114.24208950984"))
        }
    }
    ...
    private fun String.summaryValue(name: String): String {
        return split(", ")
            .first { it.startsWith("$name=") }
            .substringAfter("=")
    }
