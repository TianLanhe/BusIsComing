...
常用結果使用 `ListAdapter` 與 DiffUtil 表達跨區移動及內容更新。滑動約達卡片寬度 40% 才觸發，不使用 fling 捷徑；卡片 1:1 跟手，單次跨門檻只震動一次，未觸發時約 210ms 回彈。ETA 文字區與 48dp 鈴鐺區不啟動置頂滑動。置頂或取消會按穩定 item identity 恢復第一個可見項目與像素偏移，不自動捲頂。
