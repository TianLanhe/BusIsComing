...
- 搜尋候選：在 `360dp`、font scale 1.0、常見 IME 下驗證 5 至 6 個完整候選；小屏或大字體只能降級為完整可見項目，不得顯示半項。候選展開時在列表內向上、向下及越過頂／底滑動，確認 AppBar、頁面、結果列表和下拉刷新不移動；關閉候選後確認外層滾動及刷新資格恢復。
...
- 「本次行程」與保存：成功的非空查詢折疊為 `起點 → 終點` 上下文列；編輯未改可取消並保留結果／保存資格，實際修改立即失效。正常字體且空間足夠時可單列，`360dp`、英文或 font scale `1.3／2.0` 時路徑與操作可分兩列；核心文字不縮小、操作不裁切且各有至少 `48dp` 觸控目標。驗證命名、重名、取消／失敗重試、成功後防重複與新查詢重設保存狀態。
